    ...
    private var captionCard: some View {
        ZStack {
            ...
            if state.regenerating {
                HStack(spacing: 8) {
                    ListdSpinner(color: theme.accent)
                    Text("Rewriting…")
                        .font(ListdFont.ui(12))
                        .foregroundStyle(theme.text)
                }
            ...
            HStack {
                ...
                if !state.chips.isEmpty {
                    Button("Reset", action: state.resetChips)
                        .buttonStyle(.plain)
                        .font(ListdFont.ui(12, weight: .medium))
                        .foregroundStyle(theme.accent)
                }
            }
            .padding(.horizontal, 22)
        ...
        ZStack {
            ...
            VStack(spacing: 18) {
                Text(c.source == .camera ? "CAPTURING" : "UPLOADING")
                    .font(ListdFont.mono(11))
                    .tracking(1.1)
                    .foregroundStyle(.white.opacity(0.6))
                Text(c.label)
                    .font(ListdFont.serif(22))
                    .foregroundStyle(.white)
                if c.source == .camera {
                    Circle()
