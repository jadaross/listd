...
Native SwiftUI app — port of the `Listd iOS Prototype` design (Claude Design handoff bundle).
...
3. Settings:
   ...
   - **Language:** Swift
   - **Storage:** None
   - Bundle identifier: e.g. `com.listd.app`
4. Save location: this `ios/` directory (so the project sits at `ios/Listd.xcodeproj`)
5. In the Project Navigator, **delete the auto-generated `ContentView.swift` and `ListdApp.swift`** (move to trash) — the ones in this folder replace them.
6. Right-click the `Listd` group → **Add Files to "Listd"…** → select the `Listd/` folder in this directory → make sure "Create folder references" is selected → Add. (Xcode 16+ filesystem-synced folders work great here.)
